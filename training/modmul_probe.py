"""Interleaved modular-multiply scratchpad: learn (x*y) mod p the RIGHT way.

One-shot 5-digit x 5-digit multiply is a hard wall (tier0_probe with --hard-frac 0.5
left pure5x5 pinned at ~0.005 after 900k examples). The fix is the same one that
unlocked reduction: an explicit scratchpad with intermediate supervision -- but here
we fold multiply AND reduce into one Horner loop over y's digits, reducing mod p every
step so NO number ever exceeds ~6 digits (the regime the model already nails).

Algorithm (process y most-significant-digit first; result stays < p between digits):

    result = 0
    for each digit d of y (MSB-first):
        s  = result * 10                 # shift the accumulator   (< 10p)
        q1 = s // p ; r1 = s - q1*p       # reduce: single-digit quotient 0..9
        pp = x * d                        # single-digit multiply   (< 9p, the one new skill)
        t  = r1 + pp                      # add the partial product (< 10p)
        q2 = t // p ; r2 = t - q2*p       # reduce: single-digit quotient 0..9
        result = r2
    answer = result = (x * y) mod p

Splitting into shift-reduce / multiply / add-reduce keeps BOTH reduces at q in 0..9 --
exactly the long-division step already proven at ~0.96 -- so the only genuinely new
operation is the single-digit multiply pp = x*d (one schoolbook row, far easier than
the 5x5 cross-product that walled). All // and % live ONLY here in the data generator
that builds the supervision target; the model emits digit tokens and never divides, so
inference stays submission-compliant.

Sequence (all numbers MSB-first; fixed-order colon-separated fields per y-digit):
    BOS x MUL y MOD p EQ  d:q1:r1:pp:q2:r2 STEP d:q1:r1:pp:q2:r2 STEP ... EOS
Loss is on the whole scratchpad (everything after EQ). The final answer is r2 of the
last block = the digits after the LAST colon, exactly like longdiv_probe.

Usage:
    python training/modmul_probe.py --steps 80000 --amp --p-min 512 --p-max 65536
"""
...
PAD, BOS, MUL, MOD, EQ, COLON, STEP, EOS = 10, 11, 12, 13, 14, 15, 16, 17
VOCAB = 18
...
def modmul_rows(x: int, y: int, p: int):
    """The per-y-digit (d, q1, r1, pp, q2, r2) chain; final r2 == (x*y) % p."""
    result, rows = 0, []
...
def build_example(x: int, y: int, p: int, max_len: int):
    """Return (tokens, abacus_idx, is_output) padded to max_len. Abacus index = place
    within the current number, reset to 0 after every special token. Everything after
    EQ is supervised."""
    xd, yd, pd = digits_msb(x), digits_msb(y), digits_msb(p)
    toks = [BOS] + xd + [MUL] + yd + [MOD] + pd + [EQ]
    ...
    def emit_num(n):
        for i, d in enumerate(digits_msb(n)):
            emit(d, i)

    def emit_digit(v):          # single 0..9 value
        emit(v, 0)

    for i, (d, q1, r1, pp, q2, r2) in enumerate(modmul_rows(x, y, p)):
        if i > 0:
            emit(STEP, 0)
        emit_digit(d)
...
def make_batch(batch: int, primes: list[int], max_len: int, rng, device):
    """x, y ~ U[0, p) for p sampled uniformly from the pool (the tier-3 distribution)."""
    T, A, M = [], [], []
    for _ in range(batch):
        p = primes[rng.randrange(len(primes))]
        x, y = rng.randrange(p), rng.randrange(p)
        t, ab, m = build_example(x, y, p, max_len)
...
# ---------------------------------------------------------------------------
# Model (abacus decoder, same backbone as the other probes)
# ---------------------------------------------------------------------------
...
@torch.no_grad()
def eval_answer(model, primes, n, max_len, abacus_max, rng, device):
...
def eval_teacher_forced(model, primes, n, max_len, rng, device):
    """Cheap smooth signal during warmup: (token_acc, seq_acc) over supervised
    positions. tf_tok is the leading indicator -- autoregressive acc ~= tf_tok^chain."""
    model.eval()
...
def main() -> int:
    ...
    ap.add_argument("--ckpt", type=str, default="training/checkpoints/modmul.pt")
    ...
    # Sequence sizing: y up to pd digits -> pd blocks; pp = x*d < 9*p_max; r < p_max.
    pd = len(str(args.p_max - 1))                 # digits in a prime / operand / remainder
    ppd = len(str(9 * (args.p_max - 1)))          # max digits in a partial product pp
    abacus_max = max(ppd, pd) + 2
    # per block: d : q1 : r1 : pp : q2 : r2  (+ STEP); fields q1,q2,d are single digits
    block = 1 + 1 + 1 + 1 + pd + 1 + ppd + 1 + 1 + 1 + pd + 1
    ...
    for step in range(start_step + 1, args.steps + 1):
        ...
        if step % args.eval_every == 0:
            ...
            for lo, hi, ps in buckets:
                acc = eval_answer(model, ps, args.eval_n, max_len, abacus_max,
                                  eval_rng, device)
                accs.append(acc)
